...
//        shape = inJigsaw.shape;
        ...
        var size = Std.int(image.width / sideLen);

//        maxLen = Std.int(shape.height / inJigsaw.shapeSideLen * sideLen);
        maxLen = Std.int(inJigsaw.tiles.width / size);
        colIndex = inColIndex;
        rowIndex = inRowIndex;
...
//        update();
        bmd.copyPixels(inJigsaw.tiles, new Rectangle(colIndex * maxLen, rowIndex * maxLen, maxLen, maxLen), new Point(0, 0));
    }
