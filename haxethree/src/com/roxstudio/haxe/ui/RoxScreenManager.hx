...
//    private var starttm: Float;
    ...
    public function startRootScreen(screenClassName: String, ?screenId: String, ?requestData: Dynamic) {
        this.startScreen(null, screenClassName, screenId, null, 1, requestData, RoxAnimate.NO_ANIMATE);
    }

    public function startScreen(source: RoxScreen, screenClassName: String, screenId: String,
...
//        starttm = haxe.Timer.stamp();
        if (source != null && (stack.isEmpty() || stack.first().screen.name != source.name)) {
            trace("startScreen: Illegal stack state or bad source screen, source='" + source + "',screenId=" + screenId + ",finishTo=" + finishToScreen);
            dumpStack();
            // TODO fix this
            source = null;
        }
...
//        trace("StartScreen.afterFinish, time=" + (haxe.Timer.stamp() - starttm));
        if (screenId == null) screenId = screenClassName;
        ResKeeper.currentBundle = screenId;
        var dest = screenPool.get(screenId); // check for reusable screen
        if (dest == null) {
        ...
        stack.push({ screen: dest, requestCode: requestCode, animate: animate });
...
//        trace("<<finishScreen(screen=" + screen + ",finishTo=" + finishToScreen + ",result=" + resultCode + ",data=" + resultData + ",anim=" + animate + ",isbefore=" + isBeforeStart);
        ...
        if (top == null || top.screen.name != screen.name) {
//            throw "finishScreen: Illegal stack state or bad source screen '" + top + "'";
            trace("finishScreen: Illegal stack state or bad source screen '" + top + "',screen=" + screen.name + ",finishTo=" + finishToScreen);
            dumpStack();
// TODO fix this
        }
        ...
        var toScreen: String = switch (finishToScreen) {
            case PARENT:
                stack.first().screen.name;
            case ROOT:
                stack.last().screen.name;
            case CLEAR:
                null;
            case SCREEN(name):
                var found = false;
                for (si in stack) { if (si.screen.name == name) { found = true; break; } }
                if (!found) throw "Destination screen '" + name + "' is not on stack";
                name;
        }

        while ((top = stack.first()) != null && top.screen.name != toScreen) {
...
//        trace(">>>hide: scr=" + screen + ",finish=" + finish + ",contains=" + contains(screen) + ",num=" + this.numChildren + ",first=" + this.getChildAt(0));
        ...
        if (screen.disposeAtFinish) {
            ResKeeper.disposeBundle(screen.name);
            screen.onDestroy();
        } else { // add to pool for recycling
            screenPool.set(screen.name, screen);
        }
    }
