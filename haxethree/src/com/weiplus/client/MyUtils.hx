...
//        sp.addChild(prog);
    ...
    public static function showWaiting(label: String) {
    ...
    public static function loadArImage(path: String, ?maxPixels: Int) : BitmapData {
#if cpp
        var raw = ResKeeper.loadLocalImage(path);
        trace("loadArImage: path=" + path + ",raw="+raw.width+","+raw.height);
        if (raw == null) return null;

        var bmdw = Std.int(raw.width / 2);
        var bmd = new BitmapData(bmdw, raw.height, true, 0);
        var alpha = new BitmapData(bmdw, bmd.height, true, 0);
        bmd.copyPixels(raw, new Rectangle(0, 0, bmdw, bmd.height), new Point(0, 0));
        alpha.copyPixels(raw, new Rectangle(bmdw, 0, bmdw, bmd.height), new Point(0, 0));
        bmd.copyChannel(alpha, new Rectangle(0, 0, bmdw, alpha.height), new Point(0, 0), BitmapDataChannel.RED, BitmapDataChannel.ALPHA);
        alpha.dispose();
        raw.dispose();
        if (maxPixels > 0 && bmdw * bmd.height > maxPixels) { // too large
            var ratio = Math.sqrt(maxPixels / (bmdw * bmd.height));
            var newbmd = new BitmapData(Std.int(bmdw * ratio), Std.int(bmd.height * ratio), true, 0);
