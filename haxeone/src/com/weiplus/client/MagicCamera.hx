...
//                folderList.parent.visible = false;
            ...
            MyUtils.asyncImage(c.icon, function(bmd) {
                if (bmd == null || bmd.width == 0) {
                    trace("load AR folder failed, id=" + c.id + ",url=" + c.icon);
                    return;
                }
                folder.ico = new Bmp();
                folder.ico.bitmapData = BmdUtil.transform(bmd, "resize", [ (130).dp(), (100).dp(), BmdUtil.RESIZE_USE_MARGIN ]);
                folder.refresh();
            });
        }
    ...
    private function arLoaded(code: Int, data: Dynamic, cacheFilename: String, updateCacheOnly: Bool) {
        ...
        btn.ico = new Bmp();
        btn.ico.bitmapData = BmdUtil.transform(ResKeeper.getAssetImage("res/icon_ar_prev.png"), "resize", [ (80).dp(), (80).dp(), BmdUtil.RESIZE_USE_MARGIN ]);
...
//            w="(140).dp()" h="(140).dp()" skin:Paint-border="1" skin:Paint-color="0x00FF00" text="'folder'" />
            ...
            var bundleId = "ar_folder_" + currentCid;
            MyUtils.asyncImage(c.image, function(bmd) {
                if (bmd == null || bmd.width == 0) {
                    trace("load AR failed, id=" + c.id + ",url=" + c.image);
                    return;
                }
                btn.ico = new Bmp();
                btn.ico.bitmapData = BmdUtil.transform(bmd, "resize", [ (120).dp(), (120).dp(), BmdUtil.RESIZE_USE_MARGIN ]);
                btn.addEventListener(MouseEvent.CLICK, function(_) {
                    var sp = new Sprite();
